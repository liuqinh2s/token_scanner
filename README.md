...
| K线筛 | 历史最高价≤$0.00004、前2h最高价≤$0.00002(币龄>2h)、当前价在最高价22%~80% | DexScreener (主) + GeckoTerminal (备) | 每候选 1~3 请求 |
...
2. **价格条件**：总量 10 亿，历史最高价 ≤ 0.00004
   ...
   - 币龄 > 1 小时：当前价 ≤ 0.00002
   - 币龄 > 2 小时：前 2 小时最高价 ≤ 0.00002（通过 GeckoTerminal K线精确计算）
3. **价格区间**：当前价在历史最高价的 22%~80% 之间
4. **热点新闻**（加分项，不作为筛选条件）：实时抓取社交媒体热点关键词，与代币名称/描述交叉匹配
...
4. 通过 GeckoTerminal OHLCV API 获取 K线数据，计算真实历史最高价和前2h最高价（Stage 3 K线筛）
...
| `MAX_EARLY_HIGH_PRICE` | 0.00002 | 币龄>2h时前2h最高价上限 (K线) |
| `PRICE_RATIO_LOW` | 0.22 | 当前价/最高价 下限 (22%) |
| `PRICE_RATIO_HIGH` | 0.8 | 当前价/最高价 上限 (80%) |
