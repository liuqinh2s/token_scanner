...
4. 精筛 (瞬时)
   潜伏型筛选: 持币≥80 + 进度30%~90% + 币龄≤48h + 社交≥2 + 仿盘≥20 + 1h卖出≤200 + 没在崩盘 + 没有过山车行情
   → 从存活币中找蓄势待发信号

5. 毕业通道 (瞬时)
   A. 刚毕业强势币: 上轮未毕业→本轮毕业 + 持币≥100 + 流动性≥$10k + 没在崩盘
   B. 已毕业强势币: 持币≥200 + 流动性≥$15k + 币龄≤24h + 近3轮持币持续增长≥50
   → 从存活币中找毕业后的强势代币
...
| 仿盘数 | ≥ 20 | 数据: ≥20翻倍率25%, <20仅8% |
| 1h卖出笔数 | ≤ 200 | 数据: 翻倍币翻倍前1h卖出全是0 |
| 过山车检测 | K线振幅<3x 或 回撤<50% | 已被炒过一轮, 排除 |

## 毕业通道（毕业后强势币）

从队列存活币中找毕业后的强势代币，与潜伏型精筛互补。包含两个子通道：

### A. 刚毕业通道

核心思路：捕捉毕业瞬间，第一时间介入。仅"刚毕业"币（上一轮进度 < 100%，本轮进度 = 100%）。条件全部满足（AND）：
...
核心思路：补充刚毕业通道的盲区（入队即毕业 / 毕业瞬间数据不达标的币），通过持币数持续增长识别强势币。

数据支撑：历史数据中毕业后涨>50%的好币，持币增长中位数 941，连涨≥1轮；差币持币增长中位数 0，连涨 0 轮。

条件全部满足（AND）：

| 条件 | 阈值 | 说明 |
|------|------|------|
| 已毕业 | progress = 100% | 且不是刚毕业（已被 A 通道覆盖） |
| 持币数 | ≥ 200 | 比刚毕业更高门槛，需要更强社区 |
| 流动性 | ≥ $15,000 | 比刚毕业更高，确保交易深度 |
| 币龄 | ≤ 24h | 毕业后太久的不追 |
| 近 3 轮持币累计增长 | ≥ 50 | 核心指标：持续有人买入 |
| 近 3 轮增长轮数 | ≥ 2 轮 | 不是一次性拉升，要持续增长 |
| 近 1 轮价格变化 | ≥ -10% | 允许小幅回调，但不能崩 |
...
| `QUALITY_MIN_HOLDERS` | 80 | 精筛: 持币数 ≥ 80 |
| `QUALITY_MIN_PROGRESS` | 0.30 | 精筛: 进度 ≥ 30% |
| `QUALITY_MAX_PROGRESS` | 0.90 | 精筛: 进度 < 90% |
| `QUALITY_MAX_AGE_HOURS` | 48 | 精筛: 币龄 ≤ 48h |
| `QUALITY_MIN_H_DELTA` | -5 | 精筛: 近 1 轮持币变化 ≥ -5 |
| `QUALITY_MIN_PRICE_CHANGE` | -0.30 | 精筛: 近 1 轮价格变化 ≥ -30% |
| `QUALITY_MIN_COPYCAT` | 20 | 精筛: 仿盘数 ≥ 20 |
