...
精筛后防线 (深度检查, 仅对精筛通过的少量代币):
  ...
  - 假K线检测: 15min K线无影线实体柱 ≥ 80% 或全阳线 ≥ 90% → 排除 (控盘刷量, 会瞬间归零)
"""
...
def detect_fake_candles(candles: list[list]) -> dict:
    ...
    if not candles or len(candles) < QUALITY_FAKE_CANDLE_MIN_COUNT:
        return {"fake": False, "reason": "", "ratio": 0, "total": 0,
    ...
    if valid_count < QUALITY_FAKE_CANDLE_MIN_COUNT:
        return {"fake": False, "reason": "", "ratio": 0, "total": valid_count,
                "no_wick": no_wick_count, "bullish_ratio": 0, "bullish": bullish_count}
...
def post_quality_defense(candidates: list[dict], api_key: str) -> list[dict]:
    ...
    3. 假K线检测: GeckoTerminal 15min K线 → 无影线实体柱 ≥ 80% 排除 (控盘刷量)
    """
    ...
    for t in candidates:
        ...
        # 3. 假K线检测 (GeckoTerminal 15min K线)
        # 特征: K线全是实体柱无影线 = 价格被控制, 后面会瞬间归零
        if not exclude_reason:
            try:
                candles_15m = gt_ohlcv_15min(addr, limit=12)  # 最近 3 小时的 15min K线
                fake_result = detect_fake_candles(candles_15m)
