...
精筛条件 (潜伏型筛选, 核心思路: 宁缺毋滥, 大幅收紧):
  仅未毕业币, 已毕业币不走此通道
  条件 (全部满足, AND):
  - 持币数 ≥ 80 (数据: ≥80翻倍率34% vs <80仅11%)
  - 进度 30%~90% (有真金白银, 还有上涨空间)
  - 币龄 ≤ 48h (数据: 好币可能慢热)
  - 社交媒体数 ≥ 2 (数据: 5倍+币社交中位数3)
  - 近 1 轮持币变化 ≥ -5 (没在大量流失)
  - 近 1 轮价格变化 ≥ -30% (给波动留空间, 但不能崩)
  - 仿盘数 ≥ 20 (数据: ≥20翻倍率25%, <20仅8%; 10倍币仿盘中位数407)
...
# 核心发现: 翻倍币在进度30-60%阶段 → 持币≥80+仿盘≥20+社交≥2 翻倍率57% (最强组合)
#           10倍币共性: 全部最终毕业, 仿盘数极高(中位数407), 社交中位数3
QUALITY_MIN_HOLDERS = 80               # 精筛: 持币数 ≥ 80 (数据: ≥80翻倍率34% vs <80仅11%)
QUALITY_MIN_PROGRESS = 0.30            # 精筛: 进度 ≥ 30% (有真金白银)
QUALITY_MAX_PROGRESS = 0.90            # 精筛: 进度 < 90% (还有上涨空间)
QUALITY_MAX_AGE_HOURS = 48             # 精筛: 币龄 ≤ 48h (数据: ASTEROID在40h翻倍, 好币可能慢热)
...
QUALITY_MIN_COPYCAT = 20               # 精筛: 仿盘数 ≥ 20 (数据: ≥20翻倍率25%, <20仅8%; 10倍币仿盘中位数407)
...
_bnb_usd_price: float = 600.0  # BNB/USD 实时价格, main 中更新
...
# ===================================================================
def cleanup_old_data(max_age_days: int = 28):
    """删除 data/ 目录下超过 max_age_days 天的 JSON 文件 (排除 queue.json, smart_money.json)
    保留 28 天数据: 近 2 天供前端展示, 其余供数据分析优化策略 (需要足够样本量)
...
def main():
    ...
    def _detect_source(t: dict) -> str:
        """检测代币来源平台: 优先用 source 字段, 回退到地址后缀判断"""
        src = t.get("source")
        if src:
            return src
        addr = (t.get("address") or "").lower()
        if addr.endswith("8888") or addr.endswith("7777"):
            return "flap"
        if addr.endswith("4444") or addr.endswith("ffff"):
            return "four.meme"
        return "four.meme"
    ...
    for t in quality_results:
        ...
        out_tokens.append({
            ...
            "price_change_m5": t.get("priceChangeM5", 0),
            "price_change_h1": t.get("priceChangeH1", 0),
            "price_change_h6": t.get("priceChangeH6", 0),
            "price_change_h24": t.get("priceChangeH24", 0),
    ...
    for hist_file in DATA_DIR.glob("*.json"):
        ...
        try:
            ...
            for e in hist_data.get("eliminatedThisRound", []):
                addr = e.get("address", "")
                if addr:
                    elim_48h_addrs.add(addr)
    ...
    # 清理过期数据文件 (保留 28 天, 前端只展示近 2 天, 其余供数据分析)
    cleanup_old_data(28)
